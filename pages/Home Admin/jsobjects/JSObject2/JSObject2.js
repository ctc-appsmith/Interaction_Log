export default {
    generateJWT: async () => {
        // Check if JWT is already cached and hasn't expired
        if (appsmith.store.jwt && appsmith.store.jwtExpiry > Date.now()) {
        ...
        storeValue("jwt", jwt);
        storeValue("jwtExpiry", Date.now() + 600000); // Cache expiry time is 10 minutes from now
